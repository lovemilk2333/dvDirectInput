	...
	public class Settings : UnityModManager.ModSettings
	{
		...
		public void Render()
		{
			...
			foreach (var configControl in this.configControls.Select((val, idx) => new { idx, val }))
			{
				...
				string deviceOffsetText = GUILayout.TextField(((int)configControl.val.DeviceOffset).ToString());
			...
			string rangeMinText = GUILayout.TextField(configControl.val.InputRangeMin.ToString("0.0###", System.Globalization.CultureInfo.InvariantCulture));
			if (float.TryParse(rangeMinText, System.Globalization.NumberStyles.Float, System.Globalization.CultureInfo.InvariantCulture, out float rangeMin))
			{
				if (rangeMin < 0f) rangeMin = 0f;
				else if (rangeMin > 1f) rangeMin = 1f;
				configControl.val.InputRangeMin = rangeMin;
			}
			GUILayout.Space(16);
			GUILayout.Label("Input Range Max", GUILayout.Width(100));
			string rangeMaxText = GUILayout.TextField(configControl.val.InputRangeMax.ToString("0.0###", System.Globalization.CultureInfo.InvariantCulture));
			if (float.TryParse(rangeMaxText, System.Globalization.NumberStyles.Float, System.Globalization.CultureInfo.InvariantCulture, out float rangeMax))
			{
				if (rangeMax < 0f) rangeMax = 0f;
				else if (rangeMax > 1f) rangeMax = 1f;
			...
			GUILayout.EndHorizontal();
				...
				string rateText = GUILayout.TextField(configControl.val.Rate.ToString("0.0###", System.Globalization.CultureInfo.InvariantCulture));
				if (float.TryParse(rateText, System.Globalization.NumberStyles.Float, System.Globalization.CultureInfo.InvariantCulture, out float rate))
